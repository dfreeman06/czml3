...
from traitlets import Unicode, Instance, Float, Bool
...
class Position(Widget):
    _model_name = Unicode('CZMLPositionModel').tag(sync=True)
    _model_module = Unicode(module_name).tag(sync=True)
    _model_module_version = Unicode(module_version).tag(sync=True)
    # _view_name = Unicode('CZMLPositionView').tag(sync=True)
...
class Direction(Widget):
    _model_name = Unicode('CZMLDirectionModel').tag(sync=True)
    _model_module = Unicode(module_name).tag(sync=True)
    _model_module_version = Unicode(module_version).tag(sync=True)
    # _view_name = Unicode('CZMLDirectionView').tag(sync=True)
    # _view_module = Unicode(module_name).tag(sync=True)
    # _view_module_version = Unicode(module_version).tag(sync=True)
    x = Float(default_value=0).tag(sync=True)
    y = Float(default_value=0).tag(sync=True)
...
class CZMLCamera(Widget):
    ...
    _model_module = Unicode(module_name).tag(sync=True)
    _model_module_version = Unicode(module_version).tag(sync=True)
    # _view_name = Unicode('CZMLCameraView').tag(sync=True)
    # _view_module = Unicode(module_name).tag(sync=True)
    # _view_module_version = Unicode(module_version).tag(sync=True)

    _camera = Unicode().tag(sync=True)
    # TODO should position / direction be widgets or typedtuples (namedtuple?)
    position = Instance(Position, kw={}).tag(sync=True, **widget_serialization)
    direction = Instance(Direction, kw={}).tag(sync=True, **widget_serialization)
...
class CZMLClock(Widget):
    ...
    _model_module = Unicode(module_name).tag(sync=True)
    _model_module_version = Unicode(module_version).tag(sync=True)
    # _view_name = Unicode('CZMLClockView').tag(sync=True)
    # _view_module = Unicode(module_name).tag(sync=True)
    # _view_module_version = Unicode(module_version).tag(sync=True)

    _clock = Unicode().tag(sync=True)
    #TODO Cesium using Julian Date
